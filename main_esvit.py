...
from plot import plot_augmentation_muti
...
def get_args_parser():
    ...
    parser.add_argument('--arch', default='swin_tiny', type=str,
                        choices=['cvt_tiny', 'cvt_small', 'swin_tiny', 'swin_small', 'swin_base', 'swin_large', 'swin',
                                 'vil', 'vil_1281', 'vil_2262', 'vil_14121', 'deit_tiny', 'deit_small',
                                 'vit_base'] + torchvision_archs,
    ...
    parser.add_argument('--aug-opt', type=str, default='dino_aug', metavar='NAME',
                        help='Use different data augmentation policy. [deit_aug, dino_aug, mocov2_aug, basic_aug, tumor_aug] \
                             "(default: dino_aug)')
    ...
    # * Mixup params
    parser.add_argument('--use_mixup', type=utils.bool_flag, default=False,
...
def train_esvit(args):
    ...
    # plot_augmentation_muti(data_loader)
    # check image
    ...
    if 'swin' in args.arch:
        ...
        teacher.head = DINOHead(teacher.num_features,
                                args.out_dim,
                                args.use_bn_in_head)
    ...
    if args.wandb_logger:
        ...
        wandb.watch(models=(teacher, student), log_graph=True)
    else:
        wandb_logger = None
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        log_stats = {**{f'train_{k}': v for k, v in train_stats.items()}, 'epoch': epoch + 1}

        # ============ writing logs to wandb ... ============
        if wandb_logger:
...
def train_one_epoch(student, teacher, teacher_without_ddp, dino_loss, data_loader,
    ...
    for it, (images, _) in enumerate(metric_logger.log_every(data_loader, 100, header)):
        # images type is list [v1,v2, other 8 crop images]
...
class DDINOLoss(nn.Module):
    ...
    def forward(self, student_output, teacher_output, epoch, targets_mixup):
        ...
        Cross-entropy between softmax outputs of the teacher and student networks.
        """
        # K : MLP-head-project vector length
        # T : region-level features of dimension  T: last layer feature map length
        # P: last layer feature vector length

        # s_cls_out size B(2+8) x d_out
        # s_region_out size B(2)x7^2 + B(8)x3^2 x d_out
        # s_fea size B(2)x7^2 + B(8)x3^2 x 768
        s_cls_out, s_region_out, s_fea, s_npatch = student_output
        # t_cls_out size B(2) x d_out
        # t_region_out size B(2)(7^2) x d_out
        # t_fea size B(2)(7^2) x 768
        t_cls_out, t_region_out, t_fea, t_npatch = teacher_output

        # teacher centering and sharpening
        temp = self.teacher_temp_schedule[epoch]
        t_cls = F.softmax((t_cls_out - self.center) / temp, dim=-1)  # B x T_s
        t_cls = t_cls.detach().chunk(2)  # split B(2) -> (B,B)

        t_region = F.softmax((t_region_out - self.center_grid) / temp, dim=-1)
        t_region = t_region.detach().chunk(2)
        t_fea = t_fea.chunk(2)  # list of (BxN) X 768
        ...
        for iq, q in enumerate(t_cls):
            for v in range(len(s_cls)):
                ...
                t_region_cur, t_fea_cur = t_region[iq].view(B, N, -1), t_fea[iq].view(B, N, -1)
                ...
                t_indexed_region = torch.gather(t_region_cur,
                                                1,
