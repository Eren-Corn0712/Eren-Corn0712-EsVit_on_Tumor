...
def build_dataloader(args, is_train=True):
    if args.aug_opt == 'deit_aug':
    ...
    elif args.aug_opt == 'tumor_aug':
        transform = DataAugmentationTumor(
            args.global_crops_scale,
            args.local_crops_scale,
            args.local_crops_number,
            args.local_crops_size,
        ...
        raise ValueError(f"The augmentation not imply {args.aug_opt}")
    ...
    if args.sampler == 'distributed':
        ...
        raise ValueError(f"Not support this {args.sampler}")
...
# ========== Custom Augmentation For tumor Dataset ===========
# TODO: We need to choose batter Augmentation.
# Read image PIL -> Crop -> Resize
class DataAugmentationTumor(object):
    def __init__(self, global_crops_scale, local_crops_scale, local_crops_number, local_crops_size=96):
        ...
        for l_size in local_crops_size:
            self.local_transfo.append(transforms.Compose([
                transforms.RandomResizedCrop(l_size, scale=local_crops_scale),
                Random_HF_VF,
