...
def eval_linear(args):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if epoch % args.val_freq == 0 or epoch == args.epochs - 1:
            test_stats, report, cf_matrix = validate_network(val_loader, model, linear_classifier, args.n_last_blocks,
                                                             args.avgpool_patchtokens, depths, args)
            print(
            ...
            if best_acc < test_stats["acc1"]:
                ...
                table.to_csv(os.path.join(args.output_dir, f"{epoch}_{best_acc:.2f}.csv"))

                # ============ writing logs to wandb ... ============
                if wandb_logger:
                    wandb.run.summary["best_acc"] = best_acc
                    wandb.run.log({"table_key": wandb.Table(data=table,
                                                            rows=['precision', 'recall', 'f1-score', 'support'])})

                plt.figure(figsize=(9, 6))
                sns.heatmap(cf_matrix, annot=True, fmt="d", cmap='BuGn')
                plt.xlabel("prediction")
                plt.ylabel("label (ground truth)")
                plt.savefig(os.path.join(args.output_dir, f"{epoch}_{best_acc:.2f}_cm.png"))
                del report, table, cf_matrix
                ...
                torch.save(save_dict, os.path.join(args.output_dir, f"{epoch}_{best_acc:.2f}.pth"))
                del save_dict
...
def train(model, linear_classifier, optimizer, loader, epoch, n, avgpool, depths):
    ...
    for (inp, target) in metric_logger.log_every(loader, 20, header):
        ...
        acc1, acc2 = utils.accuracy(output, target)
        del acc2  # avoid Bug
...
@torch.no_grad()
def validate_network(val_loader, model, linear_classifier, n, avgpool, depths, args):
    ...
    labels = ["benign_mass", "malignant_tumor"]
    report = classification_report(y_true=y_true, y_pred=y_pred, target_names=labels, output_dict=True)
    # confuse matrix
    cf_matrix = confusion_matrix(y_true=y_true, y_pred=y_pred)
    cf_matrix = pd.DataFrame(cf_matrix, labels, labels)

    return {k: meter.global_avg for k, meter in metric_logger.meters.items()}, report, cf_matrix
