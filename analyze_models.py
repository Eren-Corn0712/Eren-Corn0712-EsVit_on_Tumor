...
def display_instances(image, mask, fname="test", query_position=[], figsize=(4, 4), blur=False, contour=True,
                      alpha=0.5):
    ...
    if len(query_position) > 0:
        plt.plot(query_position[0], query_position[1], '-', marker='o', color='skyblue', lw=1, mec='k', mew=1,
                 markersize=20)
...
def visualize_attn(attentions, layer_id, img, args, query=0):
    ...
    # interpolate
    th_attn = nn.functional.interpolate(th_attn.unsqueeze(0), scale_factor=scale_factor, mode="nearest")[
        0].cpu().numpy()
    ...
    save_mask_dir = os.path.join(args.output_dir,
                                 "layer" + str(layer_id) + "_query" + str(query) + "/mask_th" + str(args.threshold))
    if not os.path.exists(save_mask_dir):
        os.makedirs(save_mask_dir)

    attentions_all = torchvision.utils.make_grid(attentions.unsqueeze(1), nrow=4, padding=2, pad_value=0.5,
                                                 normalize=True, scale_each=True).permute(1, 2, 0)[:, :, 0]
    ...
    for j in range(nh):
        display_instances(image, th_attn[j],
                          fname=os.path.join(save_mask_dir, "mask_th" + str(args.threshold) + f"_head_{j:02}.png"),
                          query_position=query_position, blur=False)
    ...
    print(
        f'image_list_all_compressed {image_list_all_compressed.shape} image_list_all {image_list_all.shape} {image_list_all.max()}')
    torchvision.utils.save_image(image_list_all,
                                 os.path.join(args.output_dir, f"attn_masked_all_{layer_id}_query{query}.png"))
    torchvision.utils.save_image(image_list_all_compressed, os.path.join(args.output_dir,
                                                                         f"attn_masked_all_{layer_id}_query{query}_compressed.png"))
...
def accuracy_correspondence(img0, args):
    ...
    backbone_sim_matrix = torch.matmul(F.normalize(fea1, p=2, dim=1),
                                       F.normalize(fea2, p=2, dim=1).permute(1, 0))  # B x N_s x N_t
    dense_fea_sim, dense_fea_sim_ind = backbone_sim_matrix.max(
        dim=1)  # B x N_s; collect the index in teacher for a given student feature
    ...
    for i, v in pair_idx_sim.items():  # range(dense_fea_sim_ind.shape[0]):
        if count < 10:
            ...
            distance = (((width - coords_flatten[1, i]) - pair_idx_coords[i][1]) ** 2 + (
                    coords_flatten[0, i] - pair_idx_coords[i][0]) ** 2) ** (0.5)
...
def visualize_correspondence(img0, args):
    ...
    if not args.use_saved_aug:
        ...
        torchvision.utils.save_image(torchvision.utils.make_grid(img1a, normalize=True, scale_each=True),
                                     os.path.join(save_pair_dir, "img1.png"))
        torchvision.utils.save_image(torchvision.utils.make_grid(img2a, normalize=True, scale_each=True),
                                     os.path.join(save_pair_dir, "img2.png"))
    ...
    g_sim_matrix11 = torch.matmul(F.normalize(fea1_g, p=2, dim=1),
                                  F.normalize(fea1, p=2, dim=1).permute(1, 0))  # B x N_s x N_t
    # g_sim_matrix11 = torch.matmul(fea1_g, fea1.permute(1, 0))  # B x N_s x N_t
    g_sim_sim11, g_sim_ind11 = g_sim_matrix11.max(
        dim=1)  # B x N_s; collect the index in teacher for a given student feature

    g_sim_matrix1 = torch.matmul(F.normalize(fea1_g, p=2, dim=1),
                                 F.normalize(fea2, p=2, dim=1).permute(1, 0))  # B x N_s x N_t
    g_sim_sim1, g_sim_ind1 = g_sim_matrix1.max(
        dim=1)  # B x N_s; collect the index in teacher for a given student feature

    g_sim_matrix2 = torch.matmul(F.normalize(fea1, p=2, dim=1),
                                 F.normalize(fea2_g, p=2, dim=1).permute(1, 0))  # B x N_s x N_t
    g_sim_sim2, g_sim_ind2 = g_sim_matrix2.max(
        dim=0)  # B x N_s; collect the index in teacher for a given student feature

    g_sim_matrix22 = torch.matmul(F.normalize(fea2, p=2, dim=1),
                                  F.normalize(fea2_g, p=2, dim=1).permute(1, 0))  # B x N_s x N_t
    g_sim_sim22, g_sim_ind22 = g_sim_matrix22.max(
        dim=0)  # B x N_s; collect the index in teacher for a given student feature
    ...
    plt.plot(coords_flatten[1, g_sim_ind11], coords_flatten[0, g_sim_ind11], '-', marker='o', color='skyblue', lw=1,
             mec='k', mew=1, markersize=10)
    plt.plot(margin + width + coords_flatten[0, g_sim_ind1], coords_flatten[1, g_sim_ind1], '-', marker='v',
             color='red', lw=1, mec='k', mew=1, markersize=10)
    plt.plot(margin + width + coords_flatten[0, g_sim_ind22], coords_flatten[1, g_sim_ind22], '-', marker='o',
             color='skyblue', lw=1, mec='k', mew=1, markersize=10)
    plt.plot(coords_flatten[1, g_sim_ind2], coords_flatten[0, g_sim_ind2], '-', marker='v', color='red', lw=1, mec='k',
             mew=1, markersize=10)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--arch', default='deit_small', type=str,
                        choices=['swin_tiny', 'swin_small', 'swin', 'vil', 'vil_1281', 'vil_2262', 'deit_tiny',
                                 'deit_small', 'vit_base'],
    ...
    if not os.path.isdir(args.output_dir):
        ...
        print(f"\nResult folder is on {args.output_dir}\n")
    print()
    ########################################################################
    ...
    if args.measure_correspondence:
        ...
        print(
            f'break at count {count}, with sim_value_avg value {sim_value_avg}, accuracy {accuracy_avg} distance_error {distance_error_avg} ')

        dict_results = {'count': count, 'sim_value_avg': sim_value_avg, 'accuracy_avg': accuracy_avg,
                        'distance_error_avg': distance_error_avg}
        # save
        import pickle
